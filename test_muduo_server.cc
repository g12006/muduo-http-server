...
int main() {
    ...
    server.Get("/hello", [](const HttpRequest &req, HttpResponse *rsp) {
        std::string body = "<html><body><h1>Hello, Muduo Server!</h1><p>这是一个基于Muduo库设计的高并发服务器</p></body></html>";
        rsp->SetContent(body, "text/html");
    });
    ...
    server.Post("/login", [](const HttpRequest &req, HttpResponse *rsp) {
        ...
        std::string body = "<html><body><h1>Login Result</h1><p>Username: " + username + "</p><p>Password: " + password + "</p></body></html>";
        rsp->SetContent(body, "text/html");
    });

    // 添加PUT请求处理
    server.Put("/files/(.*)", [](const HttpRequest &req, HttpResponse *rsp) {
        std::string filename = req._matches[1];
        std::string filepath = "./wwwroot/" + filename;
        bool ret = Util::WriteFile(filepath, req._body);
        if (ret) {
            rsp->SetContent("File created successfully", "text/plain");
        } else {
            rsp->_statu = 500;
            rsp->SetContent("Failed to create file", "text/plain");
        }
    });

    // 添加DELETE请求处理
    server.Delete("/files/(.*)", [](const HttpRequest &req, HttpResponse *rsp) {
        std::string filename = req._matches[1];
        std::string filepath = "./wwwroot/" + filename;
        int ret = unlink(filepath.c_str());
        if (ret == 0) {
            rsp->SetContent("File deleted successfully", "text/plain");
        } else {
            rsp->_statu = 500;
            rsp->SetContent("Failed to delete file", "text/plain");
        }
    });

    std::cout << "Server started on port 8085" << std::endl;
    server.Listen();
    return 0;
}
