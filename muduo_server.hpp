#ifndef __MUDUO_SERVER_H__
#define __MUDUO_SERVER_H__
#include <iostream>
#include <vector>
#include <string>
#include <cassert>
...
class Channel {
    private:
        ...
        std::weak_ptr<void> _tie;       // 用于绑定对象生命周期
        bool _tied;                     // 是否已绑定
    public:
        Channel(EventLoop *loop, int fd):_fd(fd), _events(0), _revents(0), _loop(loop), _tied(false) {}
        int Fd() { return _fd; }
